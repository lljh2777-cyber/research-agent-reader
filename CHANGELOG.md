...
- 草稿正文与材料快照分别保存；先预览后确认，旧窗口不能覆盖新版本，重读保留输入。追加版本支持回看、复制及未完成提交的明确恢复，异常和并发记录保留。
- 待处理中心增加知识页草稿分类，返回前重新核对准确版本。草稿只写插件目录，不创建正式知识页、不进入正式证据检索，也不调用模型。
- 13 组专项、R3 14 个／R1 13 个／知识整理 6 个回归套件及原生桌面／430 px 窄屏验收通过。测试库原有 994 个受保护文件与 14 个标签页不变；保留 3 份草稿的 12 个记录文件。详见 [R3.11 记录](docs/knowledge-drafts-r311.md)。

### Curate PDF excerpts — 0.78.0

- PDF 摘录可从详情补充到已有笔记，保留文件页码、页内位置、字节版本和独立来源声明；个人备注单独选择，不调用模型、不升级阅读深度。
- 已登记 PDF 严格核验原文包；向论文来源笔记补充时要求同一身份、明确 PDF 路径与一致版本。受管理包损坏不回退为独立文件，未登记 PDF 限于概念、方法和综合笔记。
- 复用整理预览、批次重开、冲突保护、中断恢复、撤销和摘录侧历史；预览与写入的 PDF 核对接收取消信号。
- 13 组专项、R3 13 个／R1 13 个／知识整理 6 个套件和原生桌面／窄视口验收通过。测试库原有 990 个受保护文件不变，保留 1 份验收笔记与 3 份整理／修订记录。详见 [R3.10 记录](docs/pdf-excerpt-curation-r310.md)。

### Native PDF excerpts — 0.77.0

- 原生 PDF 的同页文字可保存为摘录，保留文件哈希、文件页码、页内准确位置、上下文与个人备注；重复保存复用，备注支持冲突保护、重载与手动整理状态。
- “回到原文”核对 PDF 文件及文字层后准确选中原句；修复页视图尚未加载时的跳转错误，保留连字字符，拒绝跨页、旧版本和损坏凭据。
- 文献库按 PDF 来源类型与文件指纹核对关联，未登记来源保持独立。PDF 摘录暂不进入正式笔记补充，不调用模型或改写原文。
- 13 组专项、R3 12 个／R1 13 个／知识整理 6 个套件与原生桌面、窄视口验收通过。测试库原有 988 个受保护文件不变，保留 2 份新摘录，关闭 4 个临时 PDF 页并保留原 14 个标签页。详见 [R3.9 记录](docs/pdf-excerpts-r39.md)。
