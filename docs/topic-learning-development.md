...
当前交付为 **T0 底层服务**。插件版本保持 `0.52.2`，服务尚未被插件运行入口引用，测试库未部署新功能。现在还不能在界面中输入主题并开始学习。
...
图中的学习空间路由和公共交互层尚待 T1 接入；T0 已实现主题侧的契约、规划和存储。实际文件：
...
当前上限为每主题 128 次路线修订、每修订 128 KiB；这是目标与路线阶段的容量约束，不能直接用于持续对话。多头冲突的人工处理入口和正文增长后的节点存储需在 T1 接入前完善。新旧记录迁移仍归 R4，不迁移或双写已有阅读数据。
...
| R1 首批 | 只读文献列表、详情、继续阅读；学习入口与会话域路由准备 | 空库／旧库／混合来源可导航，打开页面零模型调用，不展示尚未可用的主题按钮 |
| T1.1 | 主题创建、历史与路线编辑器；模型选择、用量和错误展示 | 无资料可操作；修改与重启不丢失路线；冲突可查看和处理；取消有明确状态 |
| T1.2 | 提取并复用主支线、对话和导图接口；主题专用教学引擎 | 节点 ID 稳定；主线推进、支线记忆和返回位置正确；确认后路线变更不重解释旧节点 |
| T1.3 | 理解标记、学习记录导出、原生测试与真实教学审阅 | 生成不等于掌握；导出明确来源；重启与窄窗口可用；通过冻结样本后开放主题入口 |
