...
本文保留 **T0 底层服务**的架构与历史验证，当时插件版本为 `0.52.2`。后续 `0.53.0` 交付 R1 首批导航，`0.54.0` 交付 [T1.1 主题创建、历史与路线编辑器](topic-planning-t11.md)，`0.55.0` 已交付 [T1.2 讲解、主支线与导图开发预览](topic-study-t12.md)。当前可从已确认路线打开学习；下一步接 T1.3 理解标记、导出与独立教学审阅。
...
| T1.3 | 理解标记、学习记录导出、原生测试与真实教学审阅 | 生成不等于掌握；导出明确来源；重启与窄窗口可用；通过冻结样本后开放完整主题学习流程；当前 T1.2 保持开发预览 |
