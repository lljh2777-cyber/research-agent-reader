...
本文保留 **T0 底层服务**的架构与历史验证，当时插件版本为 `0.52.2`。后续 `0.53.0` 交付 R1 首批导航，`0.54.0` 交付 [T1.1 主题创建、历史与路线编辑器](topic-planning-t11.md)，`0.55.0` 交付 [T1.2 讲解、主支线与导图开发预览](topic-study-t12.md)。`0.56.0` 已交付 [T1.3 首批理解标记、导出和冻结样本](topic-records-t13.md)；下一步保留真实首次答题并完成独立教学审阅，当前不将工程检查计作教学通过。
...
T1 的起始场景现已固定为 [t1-v1 九题样本](../tests/fixtures/topic-quality/t1-v1.json)，基线摘要与状态见 [冻结观察](../tests/fixtures/topic-quality/t1-v1-observation.json)。尚未执行真实教学；准备工具仅输出步骤输入与独立审阅材料，不会调用模型或给出自动通过结论。
