...
本文保留 **T0 底层服务**的架构与历史验证，当时插件版本为 `0.52.2`。后续 `0.53.0` 交付 R1 首批导航，`0.54.0` 交付 [T1.1 主题创建、历史与路线编辑器](topic-planning-t11.md)，`0.55.0` 交付 [T1.2 讲解、主支线与导图开发预览](topic-study-t12.md)。`0.56.0` 已交付 [T1.3 首批理解标记、导出和冻结样本](topic-records-t13.md)及 [v1 首次真实答题](topic-quality-t13b.md)；`0.56.1` 完成 [T1.3C 版本兼容与 v1 / v2 同题对照](topic-teaching-t13c.md)。两版均存在教学问题，独立审阅仍待完成，不将工程检查或表达变短计作教学通过。

## 1. 用户流程与首版边界

后续进度：[T1.3D](topic-quality-t13d.md) 已在不修改 v2 规则的情况下完成旧题事实核对和新八题首次答题。新主题仍暴露概念外推，独立审阅待完成；下一步可独立接入 R1 人工记录，主题继续保持预览。
...
图中的学习空间路由已由 R1 和 T1.1 接入；T1.2 提取共用导图几何布局、祖先路径与被动 Markdown 呈现，主题教学和资料证据仍由各自服务负责。下表为 T0 契约、规划和存储文件，原生路线编辑器见 [T1.1 交付记录](topic-planning-t11.md)，持续讲解存储与界面见 [T1.2 交付记录](topic-study-t12.md)。
...
目标与路线阶段上限为每主题 128 次修订、每修订 128 KiB。T1.1 允许查看多头历史并从指定版本创建独立副本，不覆盖原冲突。T1.2 使用独立的 `topic-learning-dialogues/<topicId>/<routeDigest>/` 事件存储持续正文，固定确认后的路线；每记录最多 192 个节点、512 次事件，具体恢复与容量边界见交付记录。新旧记录迁移仍归 R4，不迁移或双写已有阅读数据。
...
| T1.2（开发预览已交付） | 共用导图几何布局与被动呈现；主题单元讲解、主支线、持续请求记录 | [0.55.0 交付记录](topic-study-t12.md)：节点与用量可恢复，支线上下文隔离，路线变更保留旧正文；工程通过不代表教学通过 |
| T1.3A（已交付） | 用户理解标记、学习记录导出、原生验证与冻结样本 | [0.56.0 记录](topic-records-t13.md)：追加自评、固定预览、保留手工编辑和检索隔离；两个目标九题的要点与输入分离 |
| T1.3B（真实运行已完成，独立审阅待办） | 真实教学答题、首次请求／响应留档与独立审阅 | [九题首次回答与开发者问题清单](topic-quality-t13b.md)已保存；参考要点未发送给模型，原答保持可追溯；事实准确、适用条件、来源诚实和连续性尚需独立裁定，继续保持开发预览 |
| T1.3C（开发实验已完成） | 规则版本化、旧历史兼容、同题首次回答对照 | [0.56.1 记录](topic-teaching-t13c.md)：v1 原答和判断要点保留；v2 未解决事实与条件错误，不据此结束预览；下一候选需额外保留未参与修订的新题 |
| T1.3D（开发检验已完成） | 旧题事实核对、未参与 v2 修订的新八题及动态题单 | [核对与运行记录](topic-quality-t13d.md)：新题仍有概念外推；不改变 v2 规则，不报告普遍提升；本轮看过的新题随后归入已知回归 |
...
主题创建、路线、主支线、导图、自评和导出的基本流程已经接通，后续先接 R1 单篇人工阅读状态，再接主要笔记选择。人工记录功能不依赖模型回答质量，可以独立交付；T1 继续保持开发预览，其事实质量与独立审阅门槛不会因此解除。

新增或变更教学规则时，先冻结用户目标、背景、期望检查点和失败反例，再保留模型实际请求与首次响应。开发者检查与独立教学／事实审阅分开，保留失败回答，不通过重写参考要点来消除错误。独立审阅是结束开发预览的门槛；此前允许依据问题做候选修订和对照实验，但不计作教学验收。后续判断修订是否有效，除旧题回归外还需预先冻结未参与规则修订的新题，不能仅靠反复优化同九题得出普遍改善结论。

T1 的起始场景固定为 [t1-v1 九题样本](../tests/fixtures/topic-quality/t1-v1.json)，历史 [冻结观察](../tests/fixtures/topic-quality/t1-v1-observation.json)保留准备时的 `not_run`。两版真实答题分别见 [v1 运行观察](../tests/fixtures/topic-quality/t1-v1-model-observation.json)和 [v2 运行观察](../tests/fixtures/topic-quality/t1-v2-model-observation.json)；准备工具本身不调用模型，也不给出自动通过结论。
