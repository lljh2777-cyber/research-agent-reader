...
更新日期：2026-09-12。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.66.0`。T0、R1 首批导航、人工阅读状态与主要笔记选择、T1.1／T1.2 开发预览、T1.3A 理解标记与导出、T1.3B 首次答题、T1.3C 规则对照和 T1.3D 新题检验已交付，历史阶段记录保留。
...
**最新进展：** `0.65.0` 已交付[详情中的来源补充与续办](paper-continuation-r2.md)。从已保存完整书目信息的文献直接进入全文或本地 PDF，按精确标识展示该文献的记录，恢复时沿用原快照并重新核对归属。详情读取不加载处理服务，明确进入后才使用原有处理流程。下一步接通已保存原文的转换与初始笔记入口，使这些阶段可分别启动与恢复；查无结果的未核验手工记录尚未开放。跨阶段待处理中心仍属于 R3，R2 尚未完成。
