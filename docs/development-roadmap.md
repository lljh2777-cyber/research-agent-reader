...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.55.0`。T0、R1 首批导航、T1.1 主题路线与 T1.2 讲解／导图开发预览已交付，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：R0 已完成[工程收尾](r0-closeout.md)，独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，R1 首批[文献导航](library-navigation-r1.md)已交付。`0.54.0` 交付 [T1.1 主题路线预览](topic-planning-t11.md)，`0.55.0` 交付 [T1.2 讲解、主支线与导图开发预览](topic-study-t12.md)。下一步接 T1.3 理解标记、导出和教学审阅。R1 人工状态及主要笔记编辑、R2–R6、T1.3–T2 尚未完成，不将工程通过计作教学或科学审阅通过。
...
R0 工程开发已收尾，T0、R1 首批导航、T1.1 与 T1.2 开发预览已完成。下一步开展 **T1.3：理解标记、学习记录导出与独立教学审阅**，先固定教学样本、评判要点与失败反例，再保留真实请求与首次响应。科学审阅保持未结项；当前开发预览不计为已验收的教学能力。各独立步骤的当前状态如下：
...
R1 首批继续保持只读导航边界，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。T1.3 的理解状态必须由用户标记，导出明确为一般知识学习记录；冻结样本的独立教学审阅通过前保持开发预览。T1 首版后再接 R1 人工阅读状态与主要笔记编辑。
