...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.61.1`。T0、R1 首批导航、人工阅读状态与主要笔记选择、T1.1／T1.2 开发预览、T1.3A 理解标记与导出、T1.3B 首次答题、T1.3C 规则对照和 T1.3D 新题检验已交付，历史阶段记录保留。
...
| 在线获取与保存 | [`fulltext/`](../src/fulltext/)、[`source-intake.ts`](../src/papers/source-intake.ts)、[`jats/`](../src/jats/)；[M4](fulltext-acquisition-m4.md)、[M5](fulltext-acquisition-m5.md) | `0.64.0` 已接通统一添加入口和已确认身份衔接；已有文献详情中的来源补全及更多阶段动作仍待接通 |
...
**最新进展：** `0.64.0` 已交付[统一添加入口](paper-intake-r2.md)，连接[仅保存书目信息](metadata-intake-r2.md)、[本地 PDF 添加与恢复](local-pdf-intake-r2.md)和在线全文获取；查询中可直接打开已核验来源，保存后沿用确认身份继续，全文重试和重载保持原确认记录。下一步在已有文献详情中接入来源补全与继续处理，复用持久书目身份，减少重开添加窗口和重复输入；逐步连接转换、笔记与登记的独立续办。查无结果的未核验手工记录尚未开放；跨阶段待处理中心仍属于 R3，R2 尚未完成。
