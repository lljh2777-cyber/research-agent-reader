...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.56.1`。T0、R1 首批导航、T1.1／T1.2 开发预览、T1.3A 理解标记与导出、T1.3B 首次答题及 T1.3C 规则版本化与对照实验已交付，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：R0 已完成[工程收尾](r0-closeout.md)，独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，R1 首批[文献导航](library-navigation-r1.md)已交付。主题路线、讲解与导图已接通，[T1.3A 用户理解标记与导出](topic-records-t13.md)、[T1.3B 首次答题](topic-quality-t13b.md)及 [T1.3C 版本兼容与真实对照](topic-teaching-t13c.md)已完成。v2 回答变短但仍有事实错误，独立教学审阅仍待完成。R1 人工状态及主要笔记编辑、R2–R6、T1 整体验收与 T2 尚未完成，不将工程通过计作教学或科学审阅通过。
...
文献依赖主线仍为 **R0 → R1 → R2 → R3 → R4**；之后 R5、R6 可分别交付，专题对照不依赖翻译。按本次用户要求，先完成独立的 **T0 底层契约**；随后开展 **R1 首批导航 → T1 主题学习首版 → R1 剩余人工状态 → R2**。T2 资料辅助与后续阶段单独排期，不作为 T1 的前置条件。这样可在统一入口基础上交付新学习方式，不重新打开已收尾的 R0。
...
R0 工程开发已收尾，T0、R1 首批导航和 T1.1–T1.3C 开发增量已完成。下一步推进 **教学错误核对、独立审阅与新题验证设计**：依据已保存的 v1 / v2 对照，优先裁定预测与目标、梯度方向、清梯度与更新、训练／验证／测试分工，以及未经支持的来源承诺。v2 没有通过教学验收，不能从回答变短推断质量提高，也不继续把九题答案写入通用提示。后续候选除旧题回归外，需预先冻结未参与修订的新题，并明确区分规则影响、模型能力和来源能力缺口。独立审阅仍是结束预览的门槛；完成前可继续有留档的开发实验，不将其计为审阅通过。各独立步骤的当前状态如下：
...
R1 首批继续保持只读导航边界，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。T1 尚需独立教学审阅及必要修正；当前用户自评和开发者初读不能替代这一门槛。T1 首版后再接 R1 人工阅读状态与主要笔记编辑。
