...
实施进度：2026-09-10 完成 [R0 前两批：纯文献聚合、独立状态与实际资料只读适配](library-r0.md)，运行版本为 `0.51.3`。已记录测试库两次扫描及文件保护基线；持久记录、完整性能与科学质量基线尚待交付，R1–R6 尚未开始。
...
下一步继续 **R0 的最小持久记录与质量基线**。三个独立步骤的当前状态如下：
...
3. **下一步：** 增加最小文献记录存储接口及旧位置实现，验证 ID 复用、人工状态、主要笔记写入、恢复与编辑冲突；补充真实科学质量样本和多格式性能基线，明确旧 MinerU 按需核验及其他批注格式兼容。接口与测试稳定后进入 R1。
