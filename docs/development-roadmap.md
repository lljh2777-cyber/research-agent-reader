...
实施进度：R0 已完成[工程收尾](r0-closeout.md)，独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，R1 首批[文献导航](library-navigation-r1.md)已交付。主题路线、讲解与导图已接通，`0.56.0` 新增 [T1.3A 用户理解标记、导出与冻结样本](topic-records-t13.md)。下一步为 T1.3B 真实答题和独立教学审阅。R1 人工状态及主要笔记编辑、R2–R6、T1.3B–T2 尚未完成，不将工程通过计作教学或科学审阅通过。
...
R0 工程开发已收尾，T0、R1 首批导航、T1.1／T1.2 开发预览和 T1.3A 已完成。下一步开展 **T1.3B：真实答题与独立教学审阅**：按已固定的两个目标、九题及失败反例保留真实请求与首次响应，评判要点不发送给被测模型。科学审阅保持未结项；当前开发预览不计为已验收的教学能力。各独立步骤的当前状态如下：
...
R1 首批继续保持只读导航边界，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。T1.3B 尚需真实运行和独立教学审阅；当前用户自评不能替代这一门槛。T1 首版后再接 R1 人工阅读状态与主要笔记编辑。
