...
实施进度：2026-09-11 完成 [R0 前两批：纯文献聚合、独立状态与实际资料只读适配](library-r0.md)，以及[第三批：持久人工决定与恢复](library-records-r0.md)，运行版本为 `0.52.0`。已记录测试库扫描、文件保护和独立进程恢复；完整性能、科学质量基线及兼容策略尚待补充，R1–R6 尚未开始。
...
5. **下一步：** 选定 3 篇合法全文，冻结每篇至少 6 个问题、来源版本和人工核对依据，建立真实科学质量基线。当前仍为全库重扫，真实大型 PDF/JATS、操作系统冷缓存和长期内存表现未验收。接口与验收稳定后进入 R1。无 ID 旧资料的显式登记入口、身份编辑和科学审阅状态写入尚未开放。
