...
实施进度：2026-09-10 完成 [R0 第一批：纯文献聚合与独立状态契约](library-r0.md)。R0 的实际资料适配、持久记录与质量基线尚待交付，R1–R6 尚未开始；现有插件运行版本仍为 `0.51.2`。
...
下一步继续 **R0 的实际资料适配**。三个独立步骤的当前状态如下：

1. **已完成：** 在 `src/library/` 定义聚合 DTO、能力、关联结果和人工状态，复用既有身份与证据类型；实现纯内存关联，并覆盖同文献多来源、无 ID 旧资料、冲突和只读零写入。[实现记录](library-r0.md)列出边界。
2. **下一步：** 适配 `SourceCatalog`、`ReadingRepository`、现有批注与 Wiki 查询；读取失败返回可见诊断，形成实际可用的数据接口，不同时改首页布局。
3. **后续：** 增加最小文献记录存储接口及旧位置实现，验证 ID 复用、人工状态写入和恢复；质量样本与只读性能基线另附记录。接口与测试稳定后进入 R1。
