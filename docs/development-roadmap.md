...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.53.0`。T0 底层服务与 R1 首批导航已交付，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：R0 已完成[工程收尾](r0-closeout.md)，当时运行基线为 `0.52.2`；独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，尚无主题 UI。`0.53.0` 已交付 [R1 首批文献列表、详情与继续阅读导航](library-navigation-r1.md)；R1 人工状态及主要笔记编辑、R2–R6、T1–T2 尚未完成，下一步接 T1.1。R0 未关闭事项按原台账跟踪，不将工程通过计作科学审阅通过。
...
R0 工程开发已收尾，T0 底层服务和 R1 首批导航已完成。下一步开展 **T1.1：主题创建、历史与路线编辑器**，随后接共享对话与导图。科学审阅保持未结项；主题服务尚未开放运行入口，不将它计为已验收的教学能力。各独立步骤的当前状态如下：
...
R1 首批继续保持只读导航边界，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。T1.1 消费已有主题会话服务和显式会话域路由；T1 首版后再接 R1 人工阅读状态与主要笔记编辑。
