...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.52.2`。本次按用户要求加入主题学习路线并开始 T0 开发，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：截至 2026-09-11，R0 的七批工程交付已完成[收尾核对](r0-closeout.md)，运行基线为 `0.52.2`。交付包括[聚合与只读适配](library-r0.md)、[持久人工决定](library-records-r0.md)、[旧资料兼容与性能基线](library-compatibility-r0.md)、[科学阅读样本](reading-quality-baseline-r0.md)、[JATS 公式修复](jats-formulas-r0.md)和[真实问答记录](reading-quality-runner-r0.md)。18 题均保存首次回答，独立人工科学审阅仍待完成；工程收尾不表示 R0 总体科学验收通过。新增 T0 已交付主题会话与路线的底层服务，尚无用户界面；R1–R6、T1–T2 尚未开始。范围与检查见[主题学习开发流程](topic-learning-development.md)，R0 未关闭事项继续按原台账跟踪。
...
R0 工程开发已收尾，本次先按新要求完成 **T0：无来源依赖的主题会话与路线服务**，下一步开展 **R1 首批导航及两类学习入口契约**，随后接入 T1。科学审阅保持未结项；新主题服务尚未接入运行入口，不将它计为已验收的教学能力。各独立步骤的当前状态如下：
...
R1 首批消费现有只读接口，提供等待、失败、取消和来源缺口提示，先验收空库、旧库、混合来源与窄窗口。按所选版本打开来源或继续已有会话；切换页面不调用模型，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。后续再接人工阅读状态与主要笔记编辑。
