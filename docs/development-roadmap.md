...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.58.0`。T0、R1 首批导航、人工阅读状态与主要笔记选择、T1.1／T1.2 开发预览、T1.3A 理解标记与导出、T1.3B 首次答题、T1.3C 规则对照和 T1.3D 新题检验已交付，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：R0 已完成[工程收尾](r0-closeout.md)，独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，R1 首批[文献导航](library-navigation-r1.md)、[人工阅读状态](library-reading-state-r1.md)与[主要笔记选择](library-primary-note-r1.md)已交付。主题路线、讲解、导图、自评与导出已接通，[T1.3C 规则对照](topic-teaching-t13c.md)及 [T1.3D 事实核对与新题检验](topic-quality-t13d.md)仍显示事实可靠性缺口，独立教学审阅待完成。下一步整理 R1 首页、文案与次级入口；R1 整体验收、R2–R6、T1 整体验收与 T2 尚未完成，不将其他工程交付计作教学或科学审阅通过。
...
R0 工程开发已收尾，T0、R1 首批导航、人工阅读状态、主要笔记选择和 T1.1–T1.3D 开发增量已完成。下一步推进 **R1 首页与次级入口整理**：围绕文献库、阅读空间和知识整理收敛入口，统一“文献深读／论文笔记”文案，保留旧 action ID、命令和历史入口。关联代码导航与混合来源库验收仍需核对，不提前宣告 R1 全部完成。主要笔记只控制导航，不合并正文或解除来源版本守卫；阅读状态不代表模型讲解完成、理解或科学审阅通过。主题方面保留新旧题错误，暂停把通用提示扩写当作事实修复；后续规则或模型候选需保留首次回答并重新验收。独立审阅仍是结束主题预览的门槛，与不依赖模型的 R1 人工功能分别推进。各独立步骤的当前状态如下：
...
R1 页面浏览继续保持只读，不自动完整核验所有旧包；仅在用户明确保存时写人工状态或主要笔记选择，不把暂定科学状态显示为已审阅。首页与次级入口、关联代码及 R1 整体收尾尚未完成。T1 尚需独立教学审阅及必要修正；当前用户自评和开发者初读不能替代这一门槛。
