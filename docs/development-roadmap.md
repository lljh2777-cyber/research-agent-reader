...
本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。本轮只更新项目文档；下列 R0–R6 均未开始实现，不迁移 Vault，不改变现有界面或数据格式。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。
...
| 科学内容样本 | 拟选 3 篇合法可用全文，覆盖 PDF/JATS、数字单位、方法条件、主图/表格和作者局限；每篇至少 6 个固定问题 | 冻结题目、原文范围和人工核对依据；记录模型、规则版本、实际输入与输出、费用及失败原因 |

科学样本在 R0 选定与建立基线，尚不声称本轮已完成。来源不能合法获得、图像未读取或扫描件无法提取时，应产生明确缺口而不是补写答案。题目可以用模型辅助整理，但最终科学判断不能仅由生成答案的模型自评。
...
下一步从 **R0 的只读文献聚合与独立状态契约**开始，建议拆成以下独立提交：

1. 在拟新增的 `src/library/` 定义聚合 DTO、能力、关联结果和人工状态；复用 `src/papers/identity.ts` 与已有来源引用结构。添加同文献多来源、无 ID 旧资料、冲突、只读零写入的内存样本。
2. 适配 `SourceCatalog`、`ReadingRepository`、现有批注与 Wiki 查询；读取失败返回可见诊断，先形成能够支持详情页的数据接口，不同时改首页布局。
3. 增加最小文献记录存储接口及旧位置实现，验证 ID 复用、人工状态写入和恢复；质量样本与只读性能基线另附记录。接口与测试稳定后进入 R1。

这三步不迁移正式 Vault，也不将建议文件中的 `_research/` 直接作为新写入根。

## 8. 文档与证据边界

本轮分析来源为用户提供的建议、上述 TypeScript 源码静态检查和仓库已有实现／验收记录。没有读取论文全文来形成科学结论，没有运行项目业务流程或真实模型，没有新增论文、研究笔记、索引或 Vault 日志；科学处理深度不适用于本次软件路线规划。

本轮只新增本文，并在 README、全文架构和入库深读历史文档中加入后续路线入口。原用户建议、原全文架构提案、测试库及既有研究内容保持不变。未来会影响目录、schema 或正式数据迁移的步骤，应依据当时具体可审阅方案执行，不能以本路线代替实际扫描与验收。
