...
**最新进展：** `0.62.0` 已交付 R2 首批「查询标识 → 核对 → 仅保存书目信息 → 文献库查看」，支持复用已有身份和保存来源追溯，详见 [R2 首批开发记录](metadata-intake-r2.md)。[本地 PDF 底层来源与保存器](local-pdf-core-r2.md)已完成，版本保持 `0.62.0`，尚未接入选择文件界面。下一步连接本地选择、身份查询、页面确认与可见恢复记录，再接通更多原文保存／已有文献动作和分阶段续办。查无结果的未核验手工记录尚未开放；跨阶段待处理中心仍属于 R3，R2 尚未完成。
...
25. **R2 本地 PDF 底层已完成：** 独立本地来源凭据与未知版本类型接入共用确认、发布、读取和恢复器；补上书目记录先保存时的身份竞争检查。9 组专项测试、既有回归与真实 PDF 隔离保存及独立进程复用通过。版本和测试部署保持 `0.62.0`，本地选择界面与可见恢复记录尚未开放，详见 [本批记录](local-pdf-core-r2.md)。
