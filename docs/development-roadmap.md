...
实施进度：截至 2026-09-11，已完成 [R0 前两批：纯文献聚合、独立状态与实际资料只读适配](library-r0.md)、[第三批：持久人工决定与恢复](library-records-r0.md)、[第四批：旧资料兼容与性能基线](library-compatibility-r0.md)，以及[第五批：科学阅读质量样本与离线证据基线](reading-quality-baseline-r0.md)。运行版本为 `0.52.1`。3 篇论文共 18 题已固定并核对证据位置，真实 JATS 失败保留在观察记录中；模型答题及独立人工复核待完成，R0 尚未整体验收，R1–R6 尚未开始。
...
6. **下一步：** 定位真实 JATS 失败并按转换器版本修复，保留历史基线；再执行冻结问题的真实模型问答与独立人工裁定。当前仍为全库重扫，真实大型 PDF/JATS、操作系统冷缓存和长期内存表现未验收。接口与验收稳定后进入 R1。无 ID 旧资料的显式登记入口、身份编辑和科学审阅状态写入尚未开放。
