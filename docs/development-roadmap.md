...
| 在线获取与保存 | [`fulltext/`](../src/fulltext/)、[`source-intake.ts`](../src/papers/source-intake.ts)、[`jats/`](../src/jats/)；[M4](fulltext-acquisition-m4.md)、[M5](fulltext-acquisition-m5.md) | `0.63.0` 已接入本地 PDF 无模型保存与独立恢复记录；统一添加入口与后续来源动作仍待接通 |
...
**最新进展：** `0.62.0` 已交付[仅保存书目信息](metadata-intake-r2.md)，`0.63.0` 已接通[本地 PDF 添加与恢复](local-pdf-intake-r2.md)：文件选择、标识查询、版本声明、实际页面确认、共用原文保存和重载续办均可用。下一步统一“添加文献”入口，把已查询的书目身份接入获取全文、补充本地版本及打开已有来源，保留每阶段独立状态与恢复入口。查无结果的未核验手工记录尚未开放；跨阶段待处理中心仍属于 R3，R2 尚未完成。
