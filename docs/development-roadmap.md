...
更新日期：2026-09-11。最初分析基线为 `codex/research-learning-map` 的 `0.51.2`，提交 `d3a6e7525f2c604ae314c13925454c63f083f554`；当前运行基线为 `0.56.0`。T0、R1 首批导航、T1.1／T1.2 开发预览及 T1.3A 理解标记与导出已交付，历史阶段记录保留。

本路线根据用户提供的产品建议、当前代码和既有验收文档制定，作为后续开发的优先级依据。路线制定时只更新项目文档；实施进度见下方记录。建议中的目录树和操作步骤属于设计输入，不构成迁移、删除或模型调用的授权。

实施进度：R0 已完成[工程收尾](r0-closeout.md)，独立人工科学审阅仍待完成。T0 已交付[主题会话与路线底层服务](topic-learning-development.md)，R1 首批[文献导航](library-navigation-r1.md)已交付。主题路线、讲解与导图已接通，`0.56.0` 新增 [T1.3A 用户理解标记、导出与冻结样本](topic-records-t13.md)。[T1.3B 真实运行与首次回答留档](topic-quality-t13b.md)已完成，九题开发者初读问题已记录；独立教学审阅仍待完成。R1 人工状态及主要笔记编辑、R2–R6、T1 整体验收与 T2 尚未完成，不将工程通过计作教学或科学审阅通过。
...
R0 工程开发已收尾，T0、R1 首批导航、T1.1／T1.2 开发预览、T1.3A 和 T1.3B 首次真实答题留档已完成。下一步推进 **T1.3B 剩余教学审阅与修正决策**：依据已保留的两组九题首次响应，分别裁定事实、条件、教学适配、来源诚实和主支线连续性。先处理概念泛化、分支偏题、训练／验证／测试分工和梯度职责混淆等初读问题；规则如有调整，建立 v2 基线并保留 v1 原答作对照。独立科学审阅仍未结项，当前开发预览不计为已验收的教学能力。各独立步骤的当前状态如下：
...
R1 首批继续保持只读导航边界，不自动完整核验所有旧包，不把暂定科学状态显示为已审阅。T1.3B 尚需独立教学审阅及必要修正；当前用户自评和开发者初读不能替代这一门槛。T1 首版后再接 R1 人工阅读状态与主要笔记编辑。
