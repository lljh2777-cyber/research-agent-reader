...
所有组使用相同语料快照。范围与交互阅读的知识库补充检索一致：`wiki/sources`、`concepts`、`methods`、`datasets`、`synthesis`、`mocs`、`projects`、`entities`、`code`、`r`、`linux`。原文包、学习导出、批注、日志和导航索引不纳入本轮，避免示例和历史生成回答影响基准。
