...
后续[第六批公式修复](jats-formulas-r0.md)已按新转换器版本处理真实公式与备用资源问题，原始失败记录保持不变；质量工具继续固定旧转换器回放，并以 `observe` 另记新版结果。下一步在冻结输入上执行实际模型答题，逐项记录未回答、数字单位、限定条件、图像提交、越界结论、模型与规则版本、实际用量及独立人工判断。人工复核尚未完成时只报告观察，不将模型自评转为科学验收。
...
后续[第七批：真实模型答题与复核材料](reading-quality-runner-r0.md)已在这份冻结输入上实施实际答题，另存模型观察。本文及原始观察的 `not_run` 保留为第五批的历史状态，独立科学复核仍待完成。
