...
export async function loadJatsDocument(storage:SourceStorage,key:string) {
	const loaded=await loadJatsSource(storage,key),pkg=buildMarkdownReaderPackage(loaded.projection.markdown,`papers/${key}/article.md`);pkg.sourceKind="jats";pkg.title=loaded.projection.title;pkg.issues=[...loaded.snapshot.validation.issues,"JATS 支持交互深读和摘要级 Wiki 草稿；没有 PDF 页码映射"];
