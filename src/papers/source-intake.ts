...
import { decodePdfSnapshot } from "../fulltext/contracts";
...
import { decodeSourceManifest, loadPdfSource, representationDigest, sealManifest, sourceVersion, SOURCE_FILES, type SourceManifest } from "../sources/pdf-package";
...
export interface AcquiredSource {snapshot:PdfSnapshot;bytes:Uint8Array;path:string;}
...
interface SavedPlan {schemaVersion:1;deviceId:string;manifest:SourceManifest;snapshot:PdfSnapshot;confirmation:SourceConfirmation;indexBefore:string|null;indexAfter:string;}
export interface SourceSavePlan {
	requestId:string;jobId:string;snapshot:PdfSnapshot;packageKey:string;paperId:string;citekey:string;warnings:string[];
...
function assertSource(source:AcquiredSource):void {
	decodePdfSnapshot(source.snapshot);
...
function indexAfter(before:string|null,manifest:SourceManifest):string {
	const marker=`<!-- rar-source:${manifest.digest} -->`,line=`- \`papers/${manifest.packageKey}/source.pdf\` — PDF 原文（${manifest.version==="version_of_record"?"出版版本":"作者接受稿"}；metadata-only） ${marker}`;
...
export class SourceIntakeService {
	...
	private validateSaved(saved:SavedPlan,snapshot:PdfSnapshot):void {
		...
		if(saved.schemaVersion!==1)throw new Error("原文保存计划版本无效");decodeSourceManifest(saved.manifest);decodePdfSnapshot(saved.snapshot);validateSourceConfirmation(saved.confirmation,saved.manifest.requestId,snapshot);
	...
	private async commit(id:string,displayedDigest:string):Promise<SourceSavePlan> {
		...
		try{
			...
			if(current.citekey!==p.view.citekey || (current.packages.length&&current.paperId!==p.view.paperId))throw new Error("论文关联或 citekey 已变化，请重新核对");
			...
			if(!saved){
				...
				const bytes=payloads(saved,source.bytes,p.png!);manifest.files=SOURCE_FILES.map((path,i)=>({path,sha256:bytesDigest(bytes[i]),byteLength:bytes[i].length}));const {digest,...unsigned}=manifest;saved.manifest=sealManifest(unsigned);saved.indexAfter=indexAfter(saved.indexBefore,saved.manifest);
			...
			const inner=new Set(SOURCE_FILES.slice(1).map(s=>s.slice(8)));for(const e of await storage.list(root+"/_source"))if(!inner.has(e.name))throw new Error("恢复目录含未知或已提交清单，请重新打开");
			for(let i=0;i<SOURCE_FILES.length;i++){const before=await storage.read(root+"/"+SOURCE_FILES[i],values[i].length);if(before && bytesDigest(before)!==bytesDigest(values[i]))throw new Error("恢复文件已修改，未覆盖："+SOURCE_FILES[i]);}
			// Ownership transaction precedes the PDF. Every write is exclusive and verified before the commit marker.
			for(const i of [1,0,2,3,4]){this.live(id);const name=root+"/"+SOURCE_FILES[i];if(!await storage.read(name,values[i].length))await storage.create(name,values[i]);const check=await storage.read(name,values[i].length);if(!check||bytesDigest(check)!==saved.manifest.files[i].sha256)throw new Error("原文文件写入后校验失败");}
