...
export class SourceCatalog {
	...
	async associate(identity:ResolvedIdentity):Promise<CatalogPlan> {
		...
		for(const p of packages){if(identityRelation(snapshot.identity.identifiers,p.identity.identifiers)==="conflict")throw new Error("原文目录中的精确标识冲突");await loadSourcePackage(this.storage,p.packageKey);}
		...
		return {paperId:[...ids][0]||"p-"+randomUUID(),citekey,packages,legacy,warnings:[...listed.warnings,...legacy.filter(p=>p.kind!=="wiki").map(p=>"旧来源版本未知，未自动替代本次来源："+p.path)]};
	}
}
