...
export interface CurationNavigation { tab?: "pending" | "stale" | "history" | "indices" | "activity"; revisionId?: string; reviewId?: string; }
export interface SavedCurationPending { kind: "reviews" | "revisions"; id: string; path: string; state: string; updated: string; digest: string; }
...
export async function readDashboardCuration(io: Pick<SourceStorage, "read" | "list">, pending?: (item: SavedCurationPending) => void, signal?: AbortSignal): Promise<DashboardCurationSummary> {
			...
			for (const entry of entries.filter(e => !e.directory && e.name.endsWith(".json")).sort((a, b) => a.name.localeCompare(b.name))) {
				...
				try {
					...
					if (kind === "reviews") {
						...
						if (record.state !== "ready" || record.suggestions.some((s: { decision: string }) => s.decision === "pending")) pending?.({ kind, id, path: record.context.target.path, state: record.state, updated: record.updated, digest: bytesDigest(raw) });
					} else {
						...
						if (record.state !== "applied" || record.needsReview) pending?.({ kind, id, path: target[0].path, state: record.needsReview ? "stale" : record.state, updated: record.updated, digest: bytesDigest(raw) });
				...
				} catch (error) { signal?.throwIfAborted(); result.issues.push(`${kind}/${entry.name}：${error instanceof Error ? error.message : String(error)}`); }
			}
		} catch (error) { signal?.throwIfAborted(); result.issues.push(`${directory}：${error instanceof Error ? error.message : String(error)}`); }
	}
