...
export const CURATION_RULE_VERSION = "curation-v1";
...
export interface CurationEvidence { id: string; kind: "paper" | "vault"; path: string; hash: string; text: string; label: string; role: string; depth: string; origins: string[]; start?: number; end?: number; page?: number; visual?: boolean; }
export interface CurationCitation { id: string; quote: string; }
export interface CurationSuggestion { id: string; kind: SuggestionKind; paragraphId: string; claim: string; text: string; reason: string; citations: CurationCitation[]; warnings: string[]; applicable: boolean; decision: "pending" | "ignored" | "applied"; }
