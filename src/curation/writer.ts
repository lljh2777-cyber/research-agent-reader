...
export class CurationWriter {
	...
	apply(preview: CurationRevision): Promise<CurationRevision> {
		return this.service.serial(async () => {
			const prior = this.service.revisions.get(preview.id); if (prior) return prior.state === "applied" ? this.finish(prior) : this.execute(prior);
			...
			await this.service.saveRevision(rebuilt); return this.execute(rebuilt);
		});
	}
	...
	private async execute(stored: CurationRevision): Promise<CurationRevision> {
