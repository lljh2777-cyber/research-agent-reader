...
export class LearningLibrary {
	...
	async documents(signal?: AbortSignal): Promise<LearningDocument[]> {
		...
		for (const [i, file] of this.app.vault.getMarkdownFiles().filter(file => /^wiki\/qa\/[^\\]+\.md$/i.test(file.path) && !file.path.split("/").some(part => part.startsWith("."))).entries()) {
			signal?.throwIfAborted(); const metadata = this.app.metadataCache.getFileCache(file)?.frontmatter || {};
			if (metadata.demo === true || ["demo", "test"].includes(metadata.purpose) || excludedSessions.has(metadata.reading_session)) continue;
