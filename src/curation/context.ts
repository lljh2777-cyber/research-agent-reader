...
export async function prepareCuration(app: App, workspace: ReadingWorkspaceService, backendFor: (session: ReadingSession) => ReadingBackend, sessionId: string, requestedIds: string[], targetPath: string, signal?: AbortSignal): Promise<CurationContext> {
	...
	const query = nodes.map(node => node!.title + " " + node!.question).join(" "); const terms = retrievalTerms(query);
	const paragraphs = curationParagraphs(text).map(paragraph => ({ ...paragraph, score: terms.filter(term => (paragraph.heading + " " + paragraph.text).toLowerCase().includes(term)).length }))
		.sort((a, b) => b.score - a.score || a.start - b.start).slice(0, 8).map(({ score: _score, ...paragraph }) => paragraph);
	...
	const context: CurationContext = { key: "", sessionId, nodeIds, learningHash, title: readingTitle(session), source: session.source, target: { path: targetPath, title, text, hash: contentHash(text), paragraphs },
