...
import { projectJats,graphicReferences,JATS_CONVERTER,type JatsProjection } from "../jats/projection";
...
export async function loadJatsSource(storage:SourceStorage,key:string) {
	...
	const assets=graphicReferences(xml).map(ref=>content.has(ref)?assetFor(ref,content.get(ref)!):{ref,issue:snapshot.artifact.includeFigures?"同版本图片缺失："+ref:"本次未请求图片："+ref}),projection=projectJats(xml,snapshot.identity,assets);
