...
export function planIngestRegistration(notePath: string, files: Record<string, string | null>, full: boolean, id: string, created: string, scope: string): RegistrationPlan {
	...
	const key = notePath.split("/").pop()!.slice(0, -3), title = scalar(m.title), doi = scalar(m.doi).replace(/^https?:\/\/doi.org\//i, "").toLowerCase();
	...
	if (full) {
		...
		const matches = rows.slice(1).filter(r => at(r, "citekey") === key || doi && at(r, "doi").toLowerCase() === doi);
		if (matches.some(r => at(r, "citekey") !== key || at(r, "doi").toLowerCase() !== doi || at(r, "title") !== title) || matches.length > 1) throw new Error("CSV 中的 citekey、DOI 或标题存在冲突，请先核对");
		if (!matches.length) {
		...
		if (match) {
			...
			if (doi && !entry.toLowerCase().includes(doi) || !entry.includes(bib(title))) throw new Error("BibTeX 中已有同名条目，请先核对标题与 DOI");
