...
export function buildIdentityUserMessage(
	...
	const pdfEvidenceLines = localPdfEvidence?.status === "available"
		? [
			...
			`- 可用于自动绑定的元数据标题：${localPdfEvidence.trustedMetadataTitle || "（无）"}`,
			`- 可用于自动绑定的第一页可见标题块：${(localPdfEvidence.firstPageTitleCandidates || []).join("；") || "（无）"}`,
			...
			"- 第一页可见文本开始（仅作为检索线索；只有上面的高置信标题块可满足自动身份门）：",
