...
export async function openJatsSave(plugin:AgentDashboardPlugin,jobId:string) {
	const service=plugin.getJatsIntakeService(),plan=await service.prepare(jobId),modal=new Modal(plugin.app);let closed=false,busy=false,completed=false;
	...
	modal.contentEl.createEl("p",{text:`正文块 ${plan.acquired.projection.blocks.length} 个；可显示图片 ${plan.acquired.projection.assets.filter(a=>a.path).length}/${plan.acquired.projection.assets.length}。保存后可在图文阅读器中进入交互深读或生成初步文章 Wiki；没有 PDF 页码同步。`});
