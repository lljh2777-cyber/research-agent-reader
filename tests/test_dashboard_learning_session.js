...
assert.match(viewSource, /activateQueryWikiView\(prompt\)/);
assert.match(viewSource, /activateMineruReaderView\(this\.sessionState\.articlePath\)/);
assert.match(viewSource, /stage\.offsetLeft - \(scroller\.clientWidth - stage\.clientWidth\) \/ 2/);
...
assert.match(styles, /\.learning-session-branch-lane\.is-above/);
assert.match(styles, /\.learning-session-branch-lane\.is-below/);
