...
module.exports = async function sourcePickerScenario(app) {
	...
	await plugin.activateReadingWorkspace({ domain: "code" }); let view = app.workspace.getLeavesOfType("research-interactive-reading").find(l => l.view.getReadingDomain() === "code").view;
	const original = new Set(app.workspace.getLeavesOfType("research-interactive-reading").flatMap(l => [...l.view.modals])), count = plugin.getReadingWorkspace().repository.sessions.size, checks = [];
	...
	try {
		...
		let modal = [...view.modals].find(m => !original.has(m)), el = modal.contentEl, input = el.querySelector('[aria-label="原文位置"]'), kind = el.querySelector("select");
		...
		modal.close(); await plugin.activateReadingWorkspace({ domain: "paper" }); view = app.workspace.getLeavesOfType("research-interactive-reading").find(l => l.view.getReadingDomain() === "paper").view;
		view.openSource(); modal = [...view.modals].find(m => !original.has(m)); el = modal.contentEl; input = el.querySelector('[aria-label="原文位置"]'); kind = el.querySelector("select");
		...
		const before = input.value; click(el, "从 Vault 选择"); await waitFor(() => document.querySelector(".reading-vault-source-modal")); kind.value = "pdf"; kind.dispatchEvent(new Event("change"));
	...
	} finally { for (const leaf of app.workspace.getLeavesOfType("research-interactive-reading")) for (const modal of leaf.view.modals) if (!original.has(modal)) modal.close(); }
};
