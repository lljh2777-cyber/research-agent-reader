...
const PROMPTS=Object.freeze({'topic-t1-v1':'topic-teaching-v1','topic-t1-v2':'topic-teaching-v2'});
const activeBaselineId=Object.keys(PROMPTS).find(id=>PROMPTS[id]===STUDY_PROMPT_VERSION);
...
function validate(spec){
 ...
 if(spec.version!==1||!Object.hasOwn(PROMPTS,spec.id)||spec.referenceStatus!=='provisional'||spec.independentReview!=='pending'||!Array.isArray(spec.samples)||spec.samples.length!==2||!Array.isArray(spec.references))fail();
...
function prepare(spec){
 validate(spec);const promptVersion=PROMPTS[spec.id],system=topicTeachingRules(promptVersion),baselineHash=sha(spec),instructionHash=sha(system);
...
function frozenBaseline(id=activeBaselineId){
 if(!Object.hasOwn(PROMPTS,id))fail();
...
if(require.main===module){
 try{
  if(process.argv[2]!=='prepare'||!path.isAbsolute(process.argv[3]||''))throw Error('Usage: node scripts/topic-quality-baseline.cjs prepare <new absolute directory outside repository> [topic-t1-v1|topic-t1-v2]');
