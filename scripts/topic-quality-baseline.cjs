...
const {topicTeachingRules,STUDY_PROMPT_VERSION}=loadReading('topic-learning/teaching-rules.ts');
const BASELINES=require('./topic-quality-suites.cjs');
const activeBaselineId=Object.keys(BASELINES).find(id=>BASELINES[id].prompt===STUDY_PROMPT_VERSION);
...
function validate(spec){
 ...
 if(spec.version!==1||!Object.hasOwn(BASELINES,spec.id)||spec.referenceStatus!=='provisional'||spec.independentReview!=='pending'||!Array.isArray(spec.samples)||spec.samples.length!==BASELINES[spec.id].samples||!Array.isArray(spec.references))fail();
 ...
 for(const sample of spec.samples){
  topicObject(sample,['id','intent','plan','steps']);if(!/^[a-z][a-z0-9-]{0,63}$/.test(sample.id)||sampleIds.has(sample.id))fail();sampleIds.add(sample.id);validateTopicIntent(sample.intent);const plan=validateTopicPlan(sample.plan),prior=new Map(),branches=new Map();let main=0;
  ...
  for(const step of sample.steps){
   ...
   if(!/^[A-Z]\d{2}$/.test(step.id)||allSteps.has(step.id)||!strings(step.expectedPoints)||!strings(step.mustNotClaim)||!Array.isArray(step.references)||step.references.some(id=>!references.has(id)))fail();allSteps.add(step.id);
 ...
 if(allSteps.size!==BASELINES[spec.id].questions)fail();return spec;
}
function prepare(spec){
 validate(spec);const promptVersion=BASELINES[spec.id].prompt,system=topicTeachingRules(promptVersion),baselineHash=sha(spec),instructionHash=sha(system);
 const inputs={version:1,baselineHash,promptVersion,instructionHash,system,mode:'sequential-topic-teaching',automaticRetries:0,
 ...
 const observation={version:1,baselineHash,instructionHash,inputHash:sha(inputs),sampleCount:spec.samples.length,questionCount:spec.samples.reduce((n,s)=>n+s.steps.length,0),modelAnswerStatus:'not_run',independentReviewStatus:'pending'};
...
function frozenBaseline(id=activeBaselineId){
 if(!Object.hasOwn(BASELINES,id))fail();
 const file=id.replace('topic-',''),spec=require('../tests/fixtures/topic-quality/'+file+'.json'),frozen=require('../tests/fixtures/topic-quality/'+file+'-observation.json'),result=prepare(spec);
 if(JSON.stringify(result.observation)!==JSON.stringify(frozen))throw Error('Frozen baseline or teaching instructions changed');
 return{spec,frozen,result};
}
module.exports={validate,prepare,sha,frozenBaseline,activeBaselineId};
if(require.main===module){
 try{
  if(process.argv[2]!=='prepare'||!path.isAbsolute(process.argv[3]||''))throw Error('Usage: node scripts/topic-quality-baseline.cjs prepare <new absolute directory outside repository> [baseline ID]');
  ...
  const {spec,result}=frozenBaseline(process.argv[4]||activeBaselineId);
